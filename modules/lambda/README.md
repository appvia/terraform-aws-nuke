...
- **EventBridge scheduling** -- one EventBridge scheduled rule per task, each passing a static JSON input payload (`task_name`, `dry_run`, `secret_arn`, `sns_topic_arn`).
...
- **Union IAM permissions** -- the Lambda execution role receives the union of all task `permission_arns` plus `secretsmanager:GetSecretValue` for all task secrets.
...
1. Reads the EventBridge input payload: `{task_name, dry_run, secret_arn, sns_topic_arn}`
